...
We have provided slots for "Configuration" - "Code Integration settings" - "Link repositories" to customize the page module for the application.

The slots currently supported are as follows:

- [ones:settings:code_integration:default:repo:link](#onessettingscode_integrationdefaultrepolink)
- [ones:settings:pipeline:default:repo:link (deprecated)](#onessettingspipelinedefaultrepolink-deprecated)

## Slots

#### ones:settings:code_integration:default:repo:link

**ONES Requirement:** v3.14.0+

**Module path：** Configuration / Code Integration settings / Link repositories / Link Repository / select repository / Open the plugin modal by clicking the next button
...
#### ones:settings:pipeline:default:repo:link (deprecated)
...
```yaml
modules:
  - id: ones-settings-code_integration-default-repo-link-JnJi
    title: GitHub
    moduleType: ones:settings:code_integration:default:repo:link
    preload: true
    manual: true
    entry: modules/ones-settings-code_integration-default-repo-link-JnJi/index.html
```
