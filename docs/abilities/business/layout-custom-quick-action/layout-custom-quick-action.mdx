...
After executing the custom shortcut button logic, the slot must be destroyed by calling the `destroyCurrentModule()` method.
...
- [Scene One: Push information to WeChat official account](./layout-custom-quick-action-example1)
