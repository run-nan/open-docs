...
In the module whose `scene` is `render`, get the component setting through `useTestReportComponentInfo`, and then render the corresponding UI according to the component setting
...
In the module where scene is render, use `useTestReportComponentInfo` to obtain the method to save the exported content, and then update the exported content according to the component setting
