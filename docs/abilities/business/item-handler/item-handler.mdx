...
The modules that currently support intercepting change events are: `Manhour`（Time spent）、`RemainingHour`（Time remaining）、`ActivityRelease`（snapshot）、`Activity`（Milestone）、`ActivityRelationLink`(Predecessor and Successor relation)、`Deliverable`（Deliverable）、`Project`（Project）、`Task`（Issue）

Event interception is triggered in the following situations:

- `add`, `update`, `delete` related operations of Graphql mutation.

  The `Project` and `Task` modules only support `add` and `update` type events

- Create new and batch new projects, and modify project field.

- Modify the time remaining and time estimated in **Worklog mode-Simple mode**.

- Issue（Scope Reference [TaskEventHandle](../issue/task-event-handler/task-event-handler.mdx#scope)）

:::note

Unless otherwise specified, all operations related to man-hours are in the Simple mode of worklog mode.
