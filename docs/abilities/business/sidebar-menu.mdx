...
After the execution is completed, a new configuration will be added under the **abilities** attribute of the `plugin.yaml` configuration file. The configuration is as follows:
...
Execute the `npx op add module` instruction under the plug-in project directory, and select: `ones:app:new`.
...
At the same time, the following templates will be added to the `/web/src/modules` directory:
...
### Step 3: Abilities are associated with slots

You need to add the `id` of the **ones:app:new** module to the `relateModule.systemMenu` of abilities, as follows:
...
abilities:
  - id: 8k5frAw6
    ...
    abilityType: SidebarMenu
    relateModule:
      systemMenu: 'ones-app-new-aBFM' // populate id here
```

:::caution notice
This ability requires the ability to be linked to the slot, so these two parts must exist in the same plugin. And need to follow the above steps to establish the relationship, otherwise the plugin will not work properly.
:::
...
### Step 5: Control the display and hiding
