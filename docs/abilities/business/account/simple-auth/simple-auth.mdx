...
- The OP tool adds files to the plugin in `{{plugin root directory}}/backend/src/simple-auth.ts`
...
`{{plugin root directory}}/backend/src/simple-auth.ts`
...
Install and enable plugin,try different `user_uuid` to vvrify the results.
