...
Modify the corresponding content of the apability in the plugin configuration file, where **abilities.config** is the configuration of the script field ability, and the value of **abilities.config.value** is the name of the script field,here it is changed to "脚本属性 1",The value of **abilities.function.calcFieldValue** is the name of the script field calculation method, and here it is changed to "countFunc".
