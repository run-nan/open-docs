...
# Plugin Script fields - Floating point number
...
When the plugin executes the install method, the issue field is created and added to the global fields list. The user can add this field to the desired issue.

<Image img={scriptFieldFloat2} />

When the plugin is unabled/uninstalled, the value of this field becomes 0, becoming a read-only field that loses its statistical function. This field is put back into use when the plugin is re-enabled/installed.
...
Modify the corresponding content of the script field in the `{{plugin root directory}}/config/plugin.yaml`, where `config` is the configuration of the script field, and the value corresponding to the 'FieldName' key is the script field's name. Change it to the name you need. Here, change it to **脚本属性 1**, the value of `calcFieldValue` corresponds to the script field method name, and you can change it to the method name you want. Here, change it to **countFunc**.
...
Modify the script field method, which defaults to the `calcFieldValues` method of `{{plugin root directory}}/backend/src/script-field-float.ts`. Note that the value in the returned parameter `returnValue` defaults to floating point, and its unit defaults to `1 / 100000`. You can modify your script field calculation logic according to the content in the comments. You can implement this method in any path at the backend of the plugin, but be careful to throw the method.
