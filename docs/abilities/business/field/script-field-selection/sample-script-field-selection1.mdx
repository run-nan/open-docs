...
And these properties conform to real constraints with each other, which are implemented using `Plugin fieldGroup` ability.(Examples[Location information FieldGroup](../plugin-fieldgroup/location-fieldgroup-scene-one.md))，this scene only provides the implementation of the script properties themselves。
