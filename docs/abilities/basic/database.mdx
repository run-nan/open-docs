...
Sometimes plugin want to persist their own data. The database ability allows each plugin to have its own independent database and provides methods to access and manipulate it. This ability is compatible with **mysql version 5.7.x** SQL syntax.
...
For the definition of specific parameters, please refer to: [@ones-op/node-database](../../reference/packages/node-database/node-database.md)
