...
Through the hook provided by the `dashboardCard` module，you can complete the related functionality of [custom card](../../../abilities/business/custom-card/custom-card.mdx)

:::caution
this module is only available in the plugin module of [custom card](../../../abilities/business/custom-card/custom-card.mdx)
:::
...
The `OPProvider` will unmount its children component when the plugin calls the function [`lifecycle.destroy`](./#destroy). It's useful when the plugin wants to delay destroying the component by config [`destroyDelayTime`](../../config/plugin.yaml#destroydelaytime) to true in the YAML.
