...
Trigger：Scope Reference [TaskEventHandle](../issue/task-event-handler/task-event-handler.mdx#scope)
...
- **Params**
  ...
  | args          | Depending on the way the event is triggered, there are the following types: <br />Add project: [add_project](#add_project) <br />time estimated: [estimated_manhour](#estimated_manhour) <br />time remaining: [remain_manhour](#remain_manhour) <br />TaskEvent: [TaskEvent](../issue/task-event-handler/task-event-handler.mdx#TaskEvent) <br />GraphQL method: map[string]interface | Y        | Change the parameters of the event |
