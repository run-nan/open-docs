...
export async function batchCheckPermission(request: PluginRequest): Promise<PluginResponse> {
    ...
    const response = await fetchONES(
        {
            ...
            body: {
                "permission_rules": [
                    {
                        "organization_uuid": "KzfDnMsq",
                        "permission_field": "table_view_permission",
                        "team_uuid": "DTkkStVS",
                        "instance_id": "c4ae3f11",
                        "user_uuid":"VaesPA3f",
                        "context": "123"
                    },{
                        "organization_uuid": "KzfDnMsq",
    ...
    // @ts-ignore
    var data  = JSON.parse(response?.body.toString())["data"] as Array
    data.forEach(v => Logger.info(v.is_permission));
    return {
        body: {
            res: 'hello world',
            requestBody: body,
...
JSON.parse(response?.body.toString())["data"]：
normal：
 [{"is_permission": true},{"is_permission": false}]
```
