...
插件自定义配置的配置项数组，你可以通过声明此配置快速提供一些插件自定义配置项，具体使用文档请查阅 [插件自定义配置项](../../abilities/basic/custom-config)。
...
插件自定义的标准权限点配置项数组，你可以通过此配置快速搭建自己的插件权限体系，具体使用文档请查阅 [插件自定义权限点](../../abilities/business/permission/customPermission/)。
...
如需要修改插件 Logo，请修改 [插件目录](../../guide/structure) 下的 `web/public/logo.svg` 文件。
...
接口的类型，具体使用详见 [接口注册](../../abilities/basic/interface-registration)。
...
权限标识，与插件 [自定义的标准权限点配置](#permission) 或插件自定义权限点能力配合使用，详情见 [插件自定义权限点](../../abilities/business/permission/customPermission/)。
...
插件子模块，由脚手架自动生成，一般无需修改，添加方法详见 [添加插槽子模块](../../guide/abilities/add-slot#step-2-add-slot-submodule)。
