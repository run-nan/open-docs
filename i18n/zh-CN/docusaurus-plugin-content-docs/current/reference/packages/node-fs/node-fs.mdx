...
node-fs 包通过封装 Node.js 的 [fs](https://nodejs.cn/dist/latest-v16.x/docs/api/fs.html) 来实现。目前，node-fs 包仅支持 fs 中 [promises](https://nodejs.cn/dist/latest-v16.x/docs/api/fs.html#promise-api) 的接口。
...
下面给出 node-fs 包所支持的 API 列表：

| node-fs    | promises                          | 说明                                                                                                                      | 支持 |
| ---------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---- |
...
#### node-fs 包中 API 的参数与 Node.js 的 fs 保持一致，但是对文件和目录参数做了以下限制：

1.目录或文件参数必须是 string 类型。如果参数不满足，将给与错误提示：

<font color="#FF0000"> "非法参数: 文件或目录的路径不是string类型"</font> <br />
<br />

2.目录或文件参数不允许超出 workspace 目录外。如果参数不满足，将给与错误提示：

<font color="#FF0000"> "非法参数: ../file21.txt 超出可访问路径范围" </font> <br />

#### 文件大小不宜太大，建议不超过 500MB。

#### 如果读写大文件，建议采用分片|分批处理。

## 示例

node-fs 各个接口的使用和 Node.js 的 [fs](https://nodejs.cn/dist/latest-v16.x/docs/api/fs.html) 基本一致。
