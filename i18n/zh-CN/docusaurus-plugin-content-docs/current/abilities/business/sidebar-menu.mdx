...
在插件工程目录下执行 `npx op add module` 指令，并选择：`ones:app:new`。
...
并且同时还会在 `/web/src/modules` 目录下新增如下模版：
...
你需要将 **ones:app:new** 模块的 `id` 添加到 abilities 的 `relateModule.systemMenu` 中，如下所示：
...
abilities:
  - id: 8k5frAw6
    ...
    abilityType: SidebarMenu
    relateModule:
      systemMenu: 'ones-app-new-aBFM' // 将 id 填充到此处
```

:::caution 注意
该能力是要求能力和插槽联动的，所以在同一个插件中，这两部分必须存在。且需要按照上述步骤建立关联关系，否则插件将不能正常工作。
:::
