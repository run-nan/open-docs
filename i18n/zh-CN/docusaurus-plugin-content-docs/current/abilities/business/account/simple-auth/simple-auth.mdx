...
- OP 工具在插件中新增文件`{{插件根目录}}/backend/src/simple-auth.ts`
...
`{{插件根目录}}/backend/src/simple-auth.ts`
...
> Use the curl command to request a simple login API to try to log in. Replace the following command with the corresponding parameters (required for all places where `{{}} `parentheses are used), and copy to the command line to use it.
