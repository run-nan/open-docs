...
开发平台提供了自定义测试报告组件的能力，当标品提供的测试报告组件不能满足需求时，使用该能力可以在测试报告和测试报告模板编辑页面中添加自定义的组件卡片。
使用该能力时，用户可以自定义的内容包括：
...
在`scene` 为`render` 的模块中，通过`useTestReportComponentInfo` 获取组件配置，然后根据组件配置渲染对应的 UI 即可
...
在`scene` 为`render` 的模块中，通过`useTestReportComponentInfo` 获取保存导出内容的方法，然后根据组件配置更新导出内容即可
...
如果自定义组件需要支持配置，那么开发者需要添加`scene` 为`setting` 的模块，在该模块中，通过`useTestReportComponentInfo` 获取组件配置，更新组件配置的方法，以及监听弹窗提交事件的方法，然后渲染编辑配置的表单 UI 即可。
...
参考[插件多语言支持](../../basic/multiple-languages.mdx)
