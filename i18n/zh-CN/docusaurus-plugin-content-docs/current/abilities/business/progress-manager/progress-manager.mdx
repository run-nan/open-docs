...
该能力为插件实现了一个进度管理器，在插件在执行下载任务或者批量操作时，可以使用该能力将任务的执行进度可视化，并且可以定义不同语言环境下的文案展示
...
| title          | 进度标题                                                                                                                     | <a href="#LanguagePkg">LanguagePkg</a> | 是   | -      |
...
**LanguagePkg**<span id="LanguagePkg"></span>
...
| resultText  | 详情内容        | <a href="#LanguagePkg">LanguagePkg</a>     | 是   | -      |
| payload     | 完成动作        | <a href="#ProcessResult">ProcessResult</a> | 是   | -      |

**ProcessResult**<span id="ProcessResult"></span>
...
#### 第三步：完成进度管理器的 done 方法，构造并传入前端需要的 peyload 参数
