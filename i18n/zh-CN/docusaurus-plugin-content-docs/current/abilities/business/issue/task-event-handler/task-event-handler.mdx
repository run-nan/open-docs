...
使用 `npx op add ability` 添加`task-event-handler`能力。添加之后会新增`backend/src/taskeventhandler.ts`文件，在配置文件中新增了以下配置。

:::tip
开发者只需要关注的**工作项类型**和**工作项属性**两个配置，支持配置多个**工作项类型**和**工作项属性**，多个配置之间以 **","** 分隔。
:::

示例配置: 表示关注**需求**、**缺陷**和**任务**的**工作项类型**，关注**截止日期**和**负责人**的**工作项属性**的更改事件。
...
- **入参介绍**
  ...
  | task_events | []<a href="#TaskEvent">TaskEvent</a> | Y        | 变更的数据   |

  **TaskEvent**<span id="TaskEvent"></span>

  | 参数                      | 类型                                           | 必填 | 说明                                                                                                                                                          |
  | ------------------------- | ---------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | task_fields               | []<a href="#TaskEventField">TaskEventField</a> | Y    | 变更属性列表                                                                                                                                                  |
  ...
  **TaskEventField**<span id="TaskEventField"></span>
...
- **示例**
  ...
  ```typescript title="backend/src/taskeventhandler.ts"
