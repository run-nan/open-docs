...
目前支持拦截变更事件的模块有：`Manhour`（登记工时）、`RemainingHour`（剩余工时）、`ActivityRelease`（快照）、`Activity`（里程碑）、`ActivityRelationLink`(前后置依赖关系)、`Deliverable`（交付物）、`Project`（项目）、`Task`（工作项）

在以下情况会触发事件拦截：

- Graphql mutation 的 `add`、`update`、`delete` 相关操作。

  其中`Project`和`Task`模块只支持`add`和`update`类型的事件

- 新建和批量新建项目，以及项目属性修改。

- 在 **工时模式-普通模式** 下修改剩余工时，预估工时。

- 工作项（范围参考[TaskEventHandle](../issue/task-event-handler/task-event-handler.mdx#使用范围)）

:::note

如无特殊说明，所有与工时相关的操作均是在工时模式为普通模式下。
