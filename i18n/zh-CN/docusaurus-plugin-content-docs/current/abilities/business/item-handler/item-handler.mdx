...
触发方式：具体参照 [TaskEventHandler](../issue/task-event-handler/task-event-handler.mdx#scope)
...
- **入参介绍**
  ...
  | args          | 根据事件触发方式的不同，会有以下几种类型: <br />新增项目：[add_project](#add_project) <br />预估工时：[estimated_manhour](#estimated_manhour) <br />剩余工时：[remain_manhour](#remain_manhour) <br />工作项：[TaskEvent](../issue/task-event-handler/task-event-handler.mdx#TaskEvent) <br />GraphQL 方式：map[string]interface | 是   | 变更事件的参数                 |
