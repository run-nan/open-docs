...
希望在工作项属性中可以设置位置信息，包括国家、省份和城市，且这些属性相互之间符合真实约束，例如选择了国家之后，省份的可选项会被限制在国家所选值的范围内。并且如果先选择区域较小的属性能够自动填充对应的上级位置信息。最后提交信息时校验位置信息的合法性。

## 解决方案

### 实现思路：

通过向工作项中添加三个单选类型的脚本属性来表达位置信息，将他们关联起来形成一个属性组并添加到工作项实体中。

属性层级关系的约束可以在获取选项值时进行判断；自动填充上级位置信息可以使用属性值联动的功能实现；合法性校验可以使用属性值校验的功能进行实现。

:::tip

示例中出现的数据库相关 API: importSQL,exec,select 等请参考 **[数据库](../../../basic/database.mdx)** 能力
