...
该能力允许插件实现了一个类型为“**浮点数**”的特殊的[工作项属性](https://guide.ones.pro/wiki/#/team/LBrdb4wE/space/6XDAYB1a/page/ChPFxP95)。脚本属性和同类型系统属性在 **ONES** 的表现形式是一样的（同样支持通过筛选器查找）。它们不同点在于，脚本属性的名称和值完全由插件中的代码决定，用户在 ONES 中不能主动修改。插件开发者可以通过实现不同的脚本属性值计算规则，完成多样的业务开发。
