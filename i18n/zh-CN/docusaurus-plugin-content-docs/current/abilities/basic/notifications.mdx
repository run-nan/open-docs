...
示例：使用 [ONES 接口注册](../business/ONES-API/ONES-API-registration)能力注册接口，在调用接口是传入用户`uuid`，向指定的用户发送测试邮件。
...
| **参数** | **类型**     | **说明**                                                                                                                    | **必填** | **默认值** |
| :------- | :----------- | :-------------------------------------------------------------------------------------------------------------------------- | :------- | :--------- | --- | --- | --- |
| Error    | ErrorMessage | 包含以下字段：<br />- Code: 固定值：`11046`<br />- Msg：固定值：`NotifyEventFailure 11046`<br />- Err：`具体错误信息`<br /> | 否       | -          |     | 是  | -   |
