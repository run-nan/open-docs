...
- 多团队组织需组织管理员在 ONES 系统中「配置中心」-「组织配置」-「自定义 OAuth 应用」进行创建，创建 OAuth 应用。
- 单团队组织需团队管理员在 ONES 系统中「配置中心」-「团队配置」-「自定义 OAuth 应用」进行创建，创建 OAuth 应用后。
...
- 应用的授权模式：
    ...
    - 组织授权模式，这个模式的应用仅可以被多团队组织的组织管理员或者单团队组织的团队管理员授权，授权后所得到的 token 代表一个虚拟身份，这个身份具有访问组织/团队内的所有数据
