...
权限校验是否依赖页面上下文，为`true`时，会在页面上下文变动时重新校验一次模块权限，否则，只在首次进入页面时校验一次模块权限，需要与插件自定义权限点能力配合使用，详情见 [插件自定义权限点](../../abilities/business/permission/custom-permission/custom-permission.mdx)。
